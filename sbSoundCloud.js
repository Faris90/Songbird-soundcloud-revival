...
// Handle artwork transformation locally in JS
var artwork = item.artwork_url || (item.user && item.user.avatar_url) || "";
if (artwork) {
  // Ensure HTTPS, upscale resolution, and wrap it through your local proxy
  artwork = artwork.replace('http://', 'https://').replace('-large.', '-t500x500.');
  artwork = SOCL_URL + "/api/image?url=" + encodeURIComponent(artwork);
}
